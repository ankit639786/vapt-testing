export const homeObjOne = {
    lightBg: false,
    lightText: true,
    lightTextDesc: true,
    topLine: 'DEFINITION',
    headline: 'Why do Vulnerability Assessment',
    description:"Vulnerability Assessment and Penetrations Testing (VAPT) or simply, Pen testing or penetration testing, is a systematic process of determining the vulnerability of an application. With the rise of hacking and other security threats, organizations are resorting to VAPT services to protect their sensitive data from external security hacks. Penetration testing involves a simulation of the process of hacking a software/application and is a form of ethical hacking that is carried out to bring out the flaws to the surface.. ",
    buttonLabel: 'See More',
    imgStart: '',
    img: 'images/svg-1.svg',
    alt: 'Credit Card'
  };
  
  
  export const homeObjThree = {
    lightBg: true,
    lightText: false,
    lightTextDesc: false,
    topLine: 'HOW OUR PROCESS WORKS',
    headline: 'Vulnerability Assessment Process',
    description: "",
    descriptionArr: [" Goals & Objectives", "Scope", " Information Gathering", " Vulnerability Detection", "Information Analysis and Planning "],
    buttonLabel: 'See More',
    imgStart: 'start',
    img: 'images/svg-7.svg',
    alt: 'Vault'
  };
  
  export const homeObjFour = {
    lightBg: false,
    lightText: true,
    lightTextDesc: true,
    topLine: 'TOOLS',
    headline: 'Tools for Vulnerability Scanning',
    description: "",
    descriptionArr: ["  Nikto2", "Netsparker", "Acunetix ", "Intruder",  "MBSA"],
    buttonLabel: 'See More',
    imgStart: '',
    img: 'images/svg-8.svg',
    alt: 'Vault'
  };

  
  export const homeObjTwo = {
    lightBg: true,
    lightText: false,
    lightTextDesc: false,
    topLine: 'TYPES',
    headline: 'Types of a vulnerability scanner',
    description: "",
    descriptionArr: ["Cloud-Based Vulnerability Scanners", "Host-Based Vulnerability Scanners", "Network-Based Vulnerability Scanners ", "Database-Based Vulnerability Scanners"],
    buttonLabel: 'Learn More',
    imgStart: '',
    img: 'images/svg-5.svg',
    alt: 'Vault'
  };
  